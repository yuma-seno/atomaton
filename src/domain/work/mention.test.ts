...
const KNOWN = ["octocat", "Some-Reviewer"];

describe("checking who an agent tried to notify", () => {
  test("a participant is left alone", () => {
    const result = escapeUnknownMentions("@octocat please look at this", KNOWN);
    expect(result.text).toBe("@octocat please look at this");
  ...
  test("with no known participants, everything is escaped", () => {
    const result = escapeUnknownMentions("@octocat look", []);
    expect(result.text).toBe("`@octocat` look");
    expect(result.escaped).toEqual(["octocat"]);
  });
